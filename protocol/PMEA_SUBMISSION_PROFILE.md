...
| Initial submission | One complete PDF for review, with authors/institutes and figures/tables embedded in the text; supplementary files uploaded separately | A 23-page complete review PDF and a separate 20-page supplementary PDF were generated and visually inspected; supplied author metadata is integrated | Pass pending author confirmation |
...
| Data availability | The journal has adopted IOP's open-data policy; a data-availability statement is required | Third-party raw-data restrictions are stated, but the code/derived-data repository and persistent DOI remain placeholders | Blocking author/release input |
...
| Generative AI disclosure | Permitted uses must be disclosed in Acknowledgements with model/version and purpose; authors retain responsibility | Disclosure moved into Acknowledgements and identifies Codex, model family, purposes, human verification, and no generative image model | Pass pending exact model/version and author confirmation |
| Conflicts/funding/authorship | Conflicts, funding, author list, roles, and corresponding-author information must be supplied | Author names/order, affiliations, emails, PANG KEREN's ORCID, no-specific-funding statement, and no-competing-interests statement are integrated; PANG KEREN's corresponding-author role is provisional; CRediT roles and MIN CHANGRONG's department/campus remain unconfirmed | Partly complete; author confirmation required |
